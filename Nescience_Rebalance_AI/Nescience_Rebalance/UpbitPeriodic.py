...
def setup():
    ...
    if configcheck != 'configured':
        ...
        if reconfig == 'yes':
            ...
            client = ccxt.upbit({'apiKey': API_KEY, 'secret': API_SECRET})
...
def balances():
    ...
    with open('upbit/balance.json', 'w') as outfile:
...
client = ccxt.upbit({'apiKey': api_key, 'secret': api_secret, 'enableRateLimit': True})
    ...
    while count < 99999:
        ...
        if count in multiples:
            ...
            if initialcheck2 != 'done':
                ...
                with open('upbit/performance.json') as json_file:
                    performance = json.load(json_file)
                with open('upbit/old.json') as json_file:
