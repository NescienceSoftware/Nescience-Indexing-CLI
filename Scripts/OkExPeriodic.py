...
import ccxt
import ctypes
ctypes.windll.kernel32.SetConsoleTitleW("OkEx")
...
def setup():
    ...
    with open('okex/config.json') as json_file:
    ...
    if configcheck != 'configured':
        ...
        if algorithm == 'THRESHOLD':
            ...
            configuration = {'assets': assets, 'threshold': threshold, 'configcheck': configcheck, 'assetnum': assetnum,
                             ...
                             'algorithm': algorithm}
            with open('okex/config.json', 'w') as outfile:
        ...
        if reconfig == 'yes':
            ...
            client = ccxt.okex3({'apiKey': API_KEY, 'secret': API_SECRET})
            ...
            with open('okex/old.json', 'w') as outfile:
...
def balances():
    ...
    with open('okex/balance.json', 'w') as outfile:
...
def prices():
    ...
    for x in range(0, assetnum):
        ...
        while attempt is False:
            ...
            time.sleep(1)
    # saves to json
    with open('okex/prices.json', 'w') as outfile:
...
def usd_value():
    ...
    with open('okex/balance.json') as json_file:
        balance = json.load(json_file)

    with open('okex/prices.json') as json_file:
...
client = ccxt.okex3({'apiKey': api_key, 'secret': api_secret, 'enableRateLimit': True})

allocation = (.99 / assetnum)

with open('okex/initial.json') as json_file:
...
if initialcheck != 'done':
    ...
    with open('okex/count.json', 'w') as outfile:
        json.dump(data2, outfile)

    with open('okex/initial.json', 'w') as outfile:
        json.dump(data, outfile)

else:
    with open('okex/count.json') as json_file:
...
if algorithm == 'THRESHOLD':
    ...
    while count < 99999:
        ...
        multiples = [n for n in range(1, 99999) if n % int((1700/5)*assetnum) == 0]
        if count in multiples:
            # Checks for previous runs and calculates gain over initial allocation
            with open('okex/performance.json') as json_file:
