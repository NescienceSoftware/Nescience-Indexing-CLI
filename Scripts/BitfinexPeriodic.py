    ...
    if configcheck != 'configured':
        ...
        if reconfig == 'yes':
            ...
            for x in range(0, assetnum):
                ...
                assets["asset{0}".format(x)] = str(assets["asset{0}".format(x)]).upper()
...
def prices():
    ...
    for x in range(0, assetnum):
        ...
        while attempt is False:
            ...
            time.sleep(1)
    # saves to json
    with open('bitfinex/prices.json', 'w') as outfile:
...
def usd_value():
    ...
    with open('bitfinex/balance.json') as json_file:
        balance = json.load(json_file)

    with open('bitfinex/prices.json') as json_file:
...
client = ccxt.bitfinex({'apiKey': api_key, 'secret': api_secret, 'enableRateLimit': True})

allocation = (.99 / assetnum)

with open('bitfinex/initial.json') as json_file:
...
if initialcheck != 'done':
    ...
    with open('bitfinex/count.json', 'w') as outfile:
        json.dump(data2, outfile)

    with open('bitfinex/initial.json', 'w') as outfile:
        json.dump(data, outfile)

else:
    with open('bitfinex/count.json') as json_file:
...
if algorithm == 'THRESHOLD':
    ...
    while count < 99999:
        ...
        if count in multiples:
            ...
            with open('bitfinex/performance.json') as json_file:
            ...
            if initialcheck2 != 'done':
                ...
                with open('bitfinex/performance.json', 'w') as outfile:
                    json.dump(data, outfile)
                # if not the initial setup, load previous iterations and calculate differences and profit
            else:
                with open('bitfinex/performance.json') as json_file:
                    performance = json.load(json_file)
                with open('bitfinex/old.json') as json_file:
