...
import ccxt
import ctypes
ctypes.windll.kernel32.SetConsoleTitleW("Liquid")
...
def setup():
    ...
    with open('liquid/config.json') as json_file:
    ...
    if configcheck != 'configured':
        ...
        if reconfig == 'yes':
            ...
            with open('liquid/old.json', 'w') as outfile:
...
def balances():
    ...
    with open('liquid/balance.json', 'w') as outfile:
...
def prices():
    ...
    for x in range(0, assetnum):
        ...
        while attempt is False:
            ...
            time.sleep(1)
    # saves to json
    with open('liquid/prices.json', 'w') as outfile:
...
def usd_value():
    ...
    with open('liquid/balance.json') as json_file:
        balance = json.load(json_file)

    with open('liquid/prices.json') as json_file:
...
client = ccxt.liquid({'apiKey': api_key, 'secret': api_secret, 'enableRateLimit': True})

allocation = (.99 / assetnum)

with open('liquid/initial.json') as json_file:
...
if initialcheck != 'done':
    ...
    with open('liquid/count.json', 'w') as outfile:
        json.dump(data2, outfile)

    with open('liquid/initial.json', 'w') as outfile:
        json.dump(data, outfile)

else:
    with open('liquid/count.json') as json_file:
...
if algorithm == 'THRESHOLD':
    ...
    while count < 99999:
        ...
        multiples = [n for n in range(1, 99999) if n % int((2200/5)*assetnum) == 0]
        if count in multiples:
            # Checks for previous runs and calculates gain over initial allocation
            with open('liquid/performance.json') as json_file:
