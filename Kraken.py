...
import ccxt
import ctypes
ctypes.windll.kernel32.SetConsoleTitleW("Kraken")
...
def setup():
    ...
    with open('kraken/config.json') as json_file:
    ...
    if configcheck != 'configured':
        ...
        if reconfig == 'yes':
            ...
            with open('kraken/old.json', 'w') as outfile:
...
def balances():
    ...
    with open('kraken/balance.json', 'w') as outfile:
...
def prices():
    ...
    for x in range(0, assetnum):
        ...
        time.sleep(3)
    # saves to json
    with open('kraken/prices.json', 'w') as outfile:
...
def usd_value():
    ...
    with open('kraken/balance.json') as json_file:
        balanceload = json.load(json_file)
    usd = {}

    for x in range(0, assetnum):
        x = str(x + 1)
        usd["usd_asset{0}".format(x)] = float(balanceload["balance_asset{0}".format(x)] * price["price_asset{0}".format(x)])
    ...
    total_usd = (usd_assets + balanceload['cash_balance'])
...
def deviation():
    ...
    with open('kraken/dev.json', 'w') as outfile:
...
client = ccxt.kraken({'apiKey': api_key, 'secret': api_secret})

allocation = (.98 / assetnum)
...
if initialcheck != 'done':
    ...
    with open('kraken/initial.json', 'w') as outfile:
        json.dump(data, outfile)

else:
    with open('kraken/count.json') as json_file:
...
while count < 99999:
    ...
    with open('kraken/dev.json') as json_file:
    ...
    for x in range(0, assetnum):
        ...
        print(
            "Asset: " + assets["asset{0}".format(x)] + " :::: " + "Current Variation: " + str(
                dev["dev_asset{0}".format(x)] * 100) + "%", flush=True)
    ...
    if count in multiples:
        ...
        if initialcheck2 != 'done':
            ...
            with open('kraken/performance.json', 'w') as outfile:
                json.dump(data, outfile)
            # if not the initial setup, load previous iterations and calculate differences and profit
        else:
            with open('kraken/performance.json') as json_file:
                performance = json.load(json_file)
            with open('kraken/old.json') as json_file:
                loadold = json.load(json_file)
            compare = {}
            old = {}
            profit = {}
            ...
            for x in range(0, assetnum):
                ...
                old["old_asset{0}".format(x)] = loadold['old']["old_asset{0}".format(x)]
            ...
            old_cash = float(loadold['old_cash'])
