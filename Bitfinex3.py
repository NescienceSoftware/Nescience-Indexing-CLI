...
import ccxt
import ctypes
ctypes.windll.kernel32.SetConsoleTitleW("Bitfinex")
...
def setup():
    ...
    if configcheck != 'configured':
        ...
        with open('bitfinex3/config.json', 'w') as outfile:
        ...
        if reconfig == 'yes':
            ...
            client = ccxt.bitfinex({'apiKey': API_KEY, 'secret': API_SECRET})
...
def prices():
    ...
    for x in range(0, assetnum):
        ...
        time.sleep(1)
    # saves to json
    with open('bitfinex3/prices.json', 'w') as outfile:
...
def usd_value():
    ...
    with open('bitfinex3/balance.json') as json_file:
        balance = json.load(json_file)

    with open('bitfinex3/prices.json') as json_file:
...
def sell_order(pair, sell_asset, current_price, asset):
    ...
    market_name = str(asset + 'USD')

    for m in markets:
        if m['symbol'] == market_name:
            minimum = float(m['limits']['amount']['min'])
...
with open('bitfinex3/initial.json') as json_file:
...
if initialcheck != 'done':
    ...
    with open('bitfinex3/count.json', 'w') as outfile:
        json.dump(data2, outfile)

    with open('bitfinex3/initial.json', 'w') as outfile:
        json.dump(data, outfile)

else:
    with open('bitfinex3/count.json') as json_file:
...
while count < 99999:
    ...
    if count in multiples:
        ...
        with open('bitfinex3/performance.json') as json_file:
        ...
        if initialcheck2 != 'done':
            ...
            with open('bitfinex3/performance.json', 'w') as outfile:
                json.dump(data, outfile)
            # if not the initial setup, load previous iterations and calculate differences and profit
        else:
            with open('bitfinex3/performance.json') as json_file:
                performance = json.load(json_file)
            with open('bitfinex3/old.json') as json_file:
